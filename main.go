...
var (
	...
	redirectRootLocation = flag.String("sish.redirectrootlocation", "https://github.com/GilgameshxZero/http-mux", "Where to redirect the root domain to")
	httpsPems            = flag.String("sish.httpspems", "ssl/", "The location of pem files for HTTPS (fullchain.pem and privkey.pem)")
	rootDomain           = flag.String("sish.domain", "", "The domain for random subdomain assignment")
	domainLen            = flag.Int("sish.subdomainlen", 3, "The length of the random subdomain to generate")
	forceRandomSubdomain = flag.Bool("sish.forcerandomsubdomain", false, "Whether or not to force a random subdomain")
	bannedSubdomains     = flag.String("sish.bannedsubdomains", "", "A comma separated list of banned subdomains")
	...
	pkLoc                = flag.String("sish.pkloc", "~/.ssh/id_rsa", "SSH server private key, used for identification of the mux server")
	...
	authKeysDir          = flag.String("sish.keysdir", "pubkeys/", "Directory for public keys for pubkey auth, used when authing clients (if enabled)")
